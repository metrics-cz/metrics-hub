...
export function LogsViewer({
  ...
  const getLogLevelStyle = (level: string) => {
    switch (level) {
      case 'debug':
        return { icon: Bug, color: 'text-gray-500', bg: 'bg-gray-50' };
      case 'info':
      ...
      <div className="flex items-center justify-between p-4 border-b bg-white">
        <div className="flex items-center space-x-4">
          <h3 className="text-lg font-semibold">
            Integration Logs
            {integrationName && <span className="text-sm text-gray-500 ml-2">({integrationName})</span>}
          </h3>
        ...
        <div className="flex items-center space-x-2">
          {showFilters && (
            <button
              ...
              className={`p-2 rounded hover:bg-gray-100 ${showFiltersPanel ? 'bg-gray-100' : ''}`}
            >
                        ...
                        {selectedLog?.id === log.id && (
                          <div className="mt-2 space-y-2">
                            ...
                            {log.structured_data && Object.keys(log.structured_data).length > 0 && (
                              <div>
                                <div className="text-xs font-medium text-gray-700 mb-1">Structured Data:</div>
