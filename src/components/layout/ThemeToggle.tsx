 ...
 if (collapsed) {
  return (
   <div className="relative" ref={dropdownRef}>
    ...
    {isOpen && (
     <div className={`absolute bg-card shadow-lg py-1 pl-4 min-w-[120px] z-[9999] ${position === 'auth-page'
      ...
      {themes.map((t) => {
       ...
       return (
        <button
         ...
         className={`w-full py-2 text-left flex items-center gap-2 hover:bg-hover text-sm ${theme === t.value ? 'text-accent' : 'text-primary'
          }`}
        >
     ...
     {themes.map((t) => {
      ...
      return (
       <button
        ...
        className={`w-full px-3 py-2 text-left flex items-center gap-2 hover:bg-hover text-sm ${theme === t.value ? 'text-accent' : 'text-primary'
         }`}
       >
