  ...
  if (!isMobileHeader && collapsed) {
    return (
      <div className="relative">
        ...
        {open && (
          <div className="absolute left-0 top-full mt-1 bg-elevated rounded shadow-lg w-36 z-50">
            {LOCALES.map(({ code, name, Flag }) => (
              <button
                key={code}
                onClick={() => changeLocale(code)}
                className={clsx(
                  'flex items-center gap-2 px-3 py-2 w-full text-left text-white hover:bg-brand-hover',
                  code === locale ? 'bg-brand-hover font-semibold' : ''
                )}
              >
    ...
    return (
      <div className="relative w-full">
        <button
          onClick={() => setOpen(!open)}
          aria-label="Change language"
          className="flex items-center w-full gap-2 px-3 py-2 justify-between text-secondary hover:bg-hover-strong"
        >
        ...
        {open && (
          <div className="absolute left-0 top-full mt-1 bg-elevated rounded shadow-lg w-40 z-50">
            {LOCALES.map(({ code, name, Flag }) => (
              <button
                key={code}
                onClick={() => changeLocale(code)}
    ...
    return (
      <div className="relative">
        <button
          onClick={() => setOpen(!open)}
          aria-label="Change language"
          className="flex items-center gap-1 px-2 py-1 border-none rounded-full hover:bg-brand-hover transition"
        >
        ...
        {open && (
          <div className="absolute right-0 top-full mt-1 bg-elevated rounded shadow-lg w-40 z-50">
            {LOCALES.map(({ code, name, Flag }) => (
              <button
                key={code}
                onClick={() => changeLocale(code)}
