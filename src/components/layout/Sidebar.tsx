 ...
 return (
  <Link
   ...
   className={clsx(
    ...
    pathname === target && !isDisabled && 'bg-brand-active text-on-brand-active'
   )}
  >
 ...
 return (
  <div className="space-y-2 px-2 mb-3 flex flex-col w-full">
   ...
   <div className="relative">
    ...
    {profileMenuOpen && (
     <div className={`absolute rounded bg-card shadow-lg py-2 z-50 ${collapsed
...
export default function Sidebar() {
 ...
 const DesktopSidebar = (
  <aside
   className={clsx(
    'h-full bg-brand text-white flex flex-col shadow-lg transition-[width] duration-300',
    collapsed ? 'w-16' : 'w-64'
   )}
  >
   <button
 ...
 const MobileSidebar = (
  <aside className="h-full bg-brand text-white flex flex-col shadow-lg w-screen max-w-screen-sm">
   <div className="flex justify-between flex-row h-14 px-4">
    <img src="/logo.png" alt="Metrics Hub logo" className="h-12 w-auto" />

    <button
     onClick={() => setMobileOpen(false)}
     className="ml-auto p-1 border-none hover:bg-brand-hover"
    >
     <X size={24} />
    </button>
   </div>
  ...
  return (
   <>
    <div className="hidden md:block">
     <aside className="h-full bg-brand text-white flex flex-col shadow-lg w-64">
      <div className="h-12 flex items-center justify-center">
       <Menu size={24} />
      </div>
     </aside>
    </div>
    <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-brand text-white flex items-center justify-between px-4 py-3 shadow">
     <Menu size={24} />
