 ...
 return (
  <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
   <div className="grid grid-cols-2 gap-4">
    <div>
     <label className="text-sm font-medium text-primary">Jméno</label>
     <input {...register('firstName')} className="mt-1 w-full form-input px-3 py-2 bg-input border-border-default text-primary" />
     {errors.firstName && (
      <p className="text-xs text-red-600">{errors.firstName.message}</p>
     )}
    </div>
    <div>
     <label className="text-sm font-medium text-primary">Příjmení</label>
     <input {...register('lastName')} className="mt-1 w-full form-input px-3 py-2 bg-input border-border-default text-primary" />
     {errors.lastName && (
   ...
   <div>
    ...
    <div className="relative">
     <input
      type="email"
      {...register('email')}
      className="mt-1 w-full form-input px-3 py-2 pr-10 bg-input border-border-default text-primary"
     />
     {emailChecking && (
   ...
   <div>
    ...
    <input
     type="password"
     {...register('password')}
     className="mt-1 w-full form-input px-3 py-2 bg-input border-border-default text-primary"
    />
     ...
     <div className="space-y-1">
      <div className="flex items-center gap-2 text-xs">
       <div className={clsx(
        ...
        pwd.length >= 8 ? 'bg-green-500 text-white' : 'bg-hover-strong text-muted'
       )}>
        {pwd.length >= 8 && '✓'}
       </div>
       <span className={pwd.length >= 8 ? 'text-green-600' : 'text-muted'}>
        Alespoň 8 znaků
       </span>
      </div>
      <div className="flex items-center gap-2 text-xs">
       <div className={clsx(
        'w-4 h-4 rounded-sm flex items-center justify-center',
        pwdRegexUpper.test(pwd) ? 'bg-green-500 text-white' : 'bg-hover-strong text-muted'
       )}>
        {pwdRegexUpper.test(pwd) && '✓'}
       </div>
       <span className={pwdRegexUpper.test(pwd) ? 'text-green-600' : 'text-muted'}>
      ...
      <div className="flex items-center gap-2 text-xs">
       <div className={clsx(
        'w-4 h-4 rounded-sm flex items-center justify-center',
        pwdRegexNumber.test(pwd) ? 'bg-green-500 text-white' : 'bg-hover-strong text-muted'
       )}>
        {pwdRegexNumber.test(pwd) && '✓'}
       </div>
       <span className={pwdRegexNumber.test(pwd) ? 'text-green-600' : 'text-muted'}>
        Alespoň jedno číslo
       </span>
      </div>
      <div className="flex items-center gap-2 text-xs">
       <div className={clsx(
        'w-4 h-4 rounded-sm flex items-center justify-center',
        pwdRegexSpecial.test(pwd) ? 'bg-green-500 text-white' : 'bg-hover-strong text-muted'
       )}>
        {pwdRegexSpecial.test(pwd) && '✓'}
       </div>
       <span className={pwdRegexSpecial.test(pwd) ? 'text-green-600' : 'text-muted'}>
   ...
   <div>
    <label className="text-sm font-medium text-primary">Potvrzení hesla</label>
    <input
     type="password"
     {...register('password2')}
     className="mt-1 w-full form-input px-3 py-2 bg-input border-border-default text-primary"
